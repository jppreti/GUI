...
public class Main extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        scene.setOnKeyPressed(e->{
            ...
            if (e.getCode()== KeyCode.LEFT) /*rotate-=10;*/ ship.setFaceAngle(ship.getFaceAngle()-10);
            if (e.getCode()== KeyCode.RIGHT) /*rotate+=10;*/ ship.setFaceAngle(ship.getFaceAngle()+10);

            ship.getShape().setRotate(ship.getFaceAngle());

            if (e.getCode()== KeyCode.UP) {
                if (ship.getFaceAngle()==0)  ship.getShape().setTranslateY(ship.getShape().getTranslateY()-10);
                if (ship.getFaceAngle()==90) ship.getShape().setTranslateX(ship.getShape().getTranslateX()+10);
                if (ship.getFaceAngle()==180) ship.getShape().setTranslateY(ship.getShape().getTranslateY()+10);
                if (ship.getFaceAngle()==270) ship.getShape().setTranslateX(ship.getShape().getTranslateX()-10);
            }

            if (e.getCode()== KeyCode.SPACE) {
                Bullet b = new Bullet();
                b.getShape().setTranslateX(ship.getShape().getTranslateX()+10);
                b.getShape().setTranslateY(ship.getShape().getTranslateY()+10);
                tela.getChildren().addAll(b.getShape());
            }
