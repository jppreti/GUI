...
public class ColorfulCircles extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		Rectangle colors =
				new Rectangle(scene.getWidth(), scene.getHeight(), new LinearGradient(0f, 1f, 1f, 0f, true, CycleMethod.NO_CYCLE,
				new Stop[]{
...
//		substituir por:
		...
		for (Node circle: circles.getChildren()) {
		  timeline.getKeyFrames().addAll(
		    ...
		    new KeyFrame(new Duration(40000), // set end pos at 40s
		      new KeyValue(circle.translateXProperty(), Math.random() * 800),
		      new KeyValue(circle.translateYProperty(), Math.random() * 600)
		    )
		); }
