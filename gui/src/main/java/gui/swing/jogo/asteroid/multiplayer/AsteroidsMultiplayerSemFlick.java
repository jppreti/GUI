...
public class AsteroidsMultiplayerSemFlick extends JPanel implements Runnable, KeyListener {
	...
	HashMap<Color, BaseVectorShape> naves = new HashMap<Color, BaseVectorShape>();

	public AsteroidsMultiplayerSemFlick() {
		super();
					...
					try {
						...
						try {
							BaseVectorShape bvs = (BaseVectorShape) ois.readObject();
								if (bvs instanceof Ship) {
									naves.put(bvs.getCor(), bvs);
								}
								
								if (bvs instanceof Bullet) {
	...
	public void paint(Graphics g) {
		...
		for (BaseVectorShape nave : naves.values()) {
			g2d.setTransform(identity);
			g2d.setColor(nave.getCor());
			g2d.translate(nave.getX(), nave.getY());
			g2d.rotate(Math.toRadians(nave.getFaceAngle()));
			g2d.fill(nave.getShape());			
		}
	...
	public static void main (String args[]){
		AsteroidsMultiplayerSemFlick jogo = new AsteroidsMultiplayerSemFlick();
